"""
etl/fact/fetch_fact_daily.py — 按交易日期拉取日行情，upsert 到 fact_daily。

拉取策略：
  - 每次 API 调用传入 trade_date，返回当日全市场数据（约 5000 条）
...
PG_DSN      = get_pg_dsn()
MAX_WORKERS = 3   # 并发线程数，避免触发限流
BATCH_SIZE  = 15  # 每批处理的日期数，批内完成后统一写库
...
def get_pending_dates(force: bool) -> tuple[list[str], int]:
    ...
    with psycopg2.connect(**PG_DSN) as conn, conn.cursor() as cur:
        ...
        cur.execute("SELECT DISTINCT to_char(trade_date, 'YYYYMMDD') FROM fact_daily")
